...
- (void)encodeWithCoder:(NSCoder *)coder {
	if ([coder allowsKeyedCoding]) {
		...
		[coder encodeInteger:count forKey:@"CTGradientElementCount"];
		[coder encodeInteger:blendingMode forKey:@"CTGradientBlendingMode"];
	}
	else
...
- (id)initWithCoder:(NSCoder *)coder {
	...
	NSUInteger count = [coder decodeIntForKey:@"CTGradientElementCount"];
...
#if TARGET_OS_IPHONE
	//if it isnt RGB we are screwed
#else
	begin = [begin colorUsingColorSpaceName:NSCalibratedRGBColorSpace];
	end = [end colorUsingColorSpaceName:NSCalibratedRGBColorSpace];
#endif
    
	[begin getRed:&color1.red
	        green:&color1.green
	         blue:&color1.blue
	        alpha:&color1.alpha];
    
	[end getRed:&color2.red
	      green:&color2.green
	       blue:&color2.blue
	      alpha:&color2.alpha];
	color1.position = 0;
...
- (CTGradient *)addColorStop:(Color *)color atPosition:(CGFloat)position {
	CTGradient *newGradient = [self copy];
	...
	[color getRed:&newGradientElement.red
...
- (Color *)colorStopAtIndex:(NSUInteger)index {
	...
	if (element != nil)
#if TARGET_OS_IPHONE
		return [Color colorWithRed:element->red
		                     green:element->green
		                      blue:element->blue
		                     alpha:element->alpha];
#else
    return [Color colorWithCalibratedRed:element->red
                                   green:element->green
                                    blue:element->blue
                                   alpha:element->alpha];
#endif
...
#if TARGET_OS_IPHONE
	return [Color colorWithRed:components[0] / components[3] //undo premultiplication that CG requires
	                     green:components[1] / components[3]
	                      blue:components[2] / components[3]
	                     alpha:components[3]];
#else
	return [Color colorWithCalibratedRed:components[0] / components[3] //undo premultiplication that CG requires
	                               green:components[1] / components[3]
	                                blue:components[2] / components[3]
	                               alpha:components[3]];
#endif
...
- (void)fillRect:(Rect)rect angle:(CGFloat)angle {
	//First Calculate where the beginning and ending points should be
	CGPoint startPoint;
	CGPoint endPoint;
    
	if (angle == 0) {   //screw the calculations - we know the answer
		startPoint = CGPointMake(MinX(rect), MinY(rect)); //right of rect
		endPoint   = CGPointMake(MaxX(rect), MinY(rect)); //left  of rect
	}
	else if (angle == 90) { //same as above
		startPoint = CGPointMake(MinX(rect), MinY(rect)); //bottom of rect
		endPoint   = CGPointMake(MinX(rect), MaxY(rect)); //top    of rect
	}
		...
		startPoint = CGPointMake(MidX(rect) - deltax, MidY(rect) - deltay);
		endPoint   = CGPointMake(MidX(rect) + deltax, MidY(rect) + deltay);
	}
...
- (void)radialFillRect:(Rect)rect {
	CGPoint startPoint, endPoint;
	CGFloat startRadius, endRadius;
	CGFloat scalex, scaley, transx, transy;
    
	startPoint = endPoint = CGPointMake(MidX(rect), MidY(rect));
    
	startRadius = -1;
	if (Height(rect) > Width(rect)) {
		scalex = Width(rect) / Height(rect);
		transx = (Height(rect) - Width(rect)) / 2;
		scaley = 1;
		transy = 1;
		endRadius = Height(rect) / 2;
	}
	else {
		scalex = 1;
		transx = 1;
		scaley = Height(rect) / Width(rect);
		transy = (Width(rect) - Height(rect)) / 2;
		endRadius = Width(rect) / 2;
	}
...
- (void)fillBezierPath:(BezierPath *)path angle:(CGFloat)angle {
	...
	[path addClip];
	[self fillRect:path.bounds angle:angle];
